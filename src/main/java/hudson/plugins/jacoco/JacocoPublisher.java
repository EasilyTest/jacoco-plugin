...
import hudson.plugins.jacoco.git.JGitUtil;
...
public class JacocoPublisher extends Recorder implements SimpleBuildStep {
    ...
    public void perform(@Nonnull Run<?, ?> run, @Nonnull FilePath filePath, @Nonnull Launcher launcher, @Nonnull TaskListener taskListener) throws InterruptedException, IOException {
        ...
        if (!basicTagPattern.equals("") && !sshRepoPattern.equals("")) {
            ...
            logger.println("[JaCoCo plugin] 开始clone历史版本：" + baseTag);
            String result=JGitUtil.cloneRepository(sshRepoPattern, tagPath, baseTag);
            logger.println("[JaCoCo plugin] clone状态: "+result);
            logger.println("[JaCoCo plugin] clone历史版本：" + baseTag + "结束,路径: "+filePath.getParent() + DiffAST.SEPARATOR + baseTag);
            if (methodInfos.isEmpty()) {
                DiffAST.diffBaseDir(filePath.toString(), tagPath);
                logger.println("[JaCoCo plugin] 基准比对变更方法数为:" + methodInfos.size());
            }
        }
